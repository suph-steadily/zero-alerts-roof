-- RUN 2026-09-23. 20-draw preflight then 2000-draw final; 27 aggregate rows each; 3392 policy clusters. See review/threshold_empirical_bootstrap_runs.json.
-- Purpose: uncertainty around each integer threshold 70-95 and conservative utility winner frequencies.
...
-- Historical comparable population: v1.2.0 issued NB homes, creation May 1-August 15 Chicago, year built >1700, age =2026-year built >=101.
-- Existing auto signatures contribute zero H/N increments and stay selected; bar 101 is their baseline.
-- Candidate grid is every integer 70-95 plus 101. Win frequencies are restricted to this grid, not a global economic optimum.
...
WITH
 arrayConcat([toUInt64(101)],arrayReverse(range(toUInt64(70),toUInt64(96)))) AS bars,
 2000 AS replicates,
 per_policy AS (
  SELECT policy_id,
  ...
  WHERE quote_type='NewBusiness' AND quote_status='Issued'
    ...
    AND pol_created_timestamp<toDateTime('2026-08-16','America/Chicago')
