...
-- Purpose: L13, A36. Repeat the within-loss-quarter comparison for BOTH peril groups.
-- Expected on original all-age population: excluded wind/hail 22/25 below control median.
-- That gives one-sided Binomial(25,0.5) p=0.0000782609; this null assumes independent
-- claims and an appropriate median, not the exclusion's causal effect.
-- Report ties separately and exclude ties from the sign-test denominator; controls'
-- estimated medians add uncertainty. Bootstrap policies and re-estimate medians.
...
,
j AS (
    SELECT *, toStartOfQuarter(loss_date) AS loss_quarter,
           if(peril IN ('Hail','Wind'), 'wind_hail', 'other_perils') AS peril_group, age_scope
    FROM claim_home
    ARRAY JOIN if(age >= 101, ['all_ages', '101plus'], ['all_ages']) AS age_scope
),
med AS (
    SELECT age_scope, peril_group, loss_quarter,
           quantileExact(0.5)(paid) AS paid_median,
           quantileExact(0.5)(incurred) AS incurred_median, count() AS controls
    FROM j WHERE rse=0 GROUP BY age_scope, peril_group, loss_quarter
)
SELECT j.age_scope, j.peril_group, count() AS excluded_claims,
       countIf(m.controls>0) AS matched_claims,
       countIf(m.controls>0 AND j.paid<m.paid_median) AS paid_below,
       countIf(m.controls>0 AND j.paid=m.paid_median) AS paid_tied,
       countIf(m.controls>0 AND j.paid>m.paid_median) AS paid_above,
       countIf(m.controls>0 AND j.incurred<m.incurred_median) AS incurred_below,
       countIf(m.controls>0 AND j.incurred=m.incurred_median) AS incurred_tied,
       countIf(m.controls>0 AND j.incurred>m.incurred_median) AS incurred_above,
       (SELECT ambiguous_claims FROM match_audit) AS ambiguous_claims
FROM j LEFT JOIN med m ON m.age_scope=j.age_scope AND m.peril_group=j.peril_group
                     AND m.loss_quarter=j.loss_quarter
WHERE j.rse=1 GROUP BY j.age_scope,j.peril_group ORDER BY j.age_scope,j.peril_group;
