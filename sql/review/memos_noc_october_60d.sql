...
-- Purpose: October 1 re-pull: full 60-day issue clock for auto and comparator, all three age bands.
-- Claims: N11 A50
-- Expected if the memo is right: At Aug20, creation-clock E3 subset had 0/186 auto; October issue-clock count is not known.
-- Scheduled analysis cutoff, not a claim that October data exists today. Run only after the cutoff is available.
...
WITH toDate('2026-10-01') AS as_of,
first_nb AS (
...
-- Actual NOC letters and reinstatement timing need a validated source.
SELECT database,table,name,type FROM system.columns
WHERE match(lower(concat(database,'.',table,'.',name)),'noc|letter|reinstat|notice')
